...
function displayActionButtons($id) {
    echo '<td>';
    echo '<a href="edit_update.php?id=' . $id . '" class="btn btn-sm btn-primary" style="background-color: #001E6C; border:none; margin-right:1rem;">Edit</a>';
    echo '<a href="hapus.php?id=' . $id . '" class="btn btn-sm btn-danger" style="background-color: #dc3545; border:none;" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\')">Hapus</a>';
    echo '</td>';
}
...
<head>
  ...
  <title>Smart Class | Data Tugas</title>
...
?>
  ...
  <div class="content-wrapper">
    ...
    <?php
        include 'koneksi.php'; 
        $query_tugas_user = "SELECT * FROM tugas WHERE id_kelas = '$id_kelas'";
$result_tugas_user = mysqli_query($koneksi, $query_tugas_user);
        ?>
      <div class="container-fluid">
        <!-- ======================================================================== -->
        ...
        <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <th style="background-color:#FCD900">Tanggal</th>
                    <th style="background-color:#FCD900">Mapel</th>
                    <th style="background-color:#FCD900">Nama tugas</th>
                    <th style="background-color:#FCD900">Tugas</th>
                    <th style="background-color:#FCD900">Tenggat</th>
                      <?php
                      if (isAdminLoggedIn()) {
                        echo "<th style='background-color:#FCD900'>aksi</th>";
                        }
                        ?>
                    </tr>
                  </thead>
                ...
                if ($_SESSION['role'] === 'user') {
                $query = "SELECT * FROM tugas where id_kelas='$_SESSION[id_kelas]' ORDER BY waktu DESC";
                
                $result = mysqli_query($koneksi, $query);

                
                while ($row_tugas_user = mysqli_fetch_assoc($result)) {
                    echo "<tr data-mapel='" . $row_tugas_user['mapel'] . "'>";
                    echo "<td>" . $id++ . "</td>";
                    echo "<td>" . $row_tugas_user['tanggal'] . "</td>";
                    echo "<td>" . $row_tugas_user['mapel'] . "</td>";
                    echo "<td>" . $row_tugas_user['namatugas'] . "</td>";
                    echo "<td>" . $row_tugas_user['tugas'] . "</td>";
                
                    //ubah badge
                    $waktuTugas = strtotime($row_tugas_user['waktu']);
                    $sekarang = time();
                    $badgeClass = $waktuTugas < $sekarang ? 'badge-secondary' : 'badge-danger';
                    echo '<td><span class="badge ' . $badgeClass . '">' . $row_tugas_user['waktu'] . '</span></td>';
                
                    //tombol aksi
                    if (isAdminLoggedIn()) {
                        displayActionButtons($row_tugas_user['id']);
                    }
                
                    echo "</tr>";
                }
              }elseif ($_SESSION['role'] === 'admin'){
                $query = "SELECT * FROM tugas where id_kelas='$_SESSION[id_kelas_admin]' ORDER BY waktu DESC";
        ...
        if ($_SESSION['role'] === 'admin') {
            ...
            // Jika pengguna sudah login sebagai admin, tampilkan tombol tambah data dan logout
            echo '<a class="btn btn-primary mb-2 mt-1  " href="tambah.php" style="width:150px; float:right;">Tambah Tugas</a>';
